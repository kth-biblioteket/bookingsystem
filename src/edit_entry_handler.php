...
//Do the kth_entry_places om det är aktiverat i config
if($entry_places_choice){
  $compare = $_POST["f_entry_places"];
  $sql = "DELETE FROM kth_entry_places 
        WHERE entry_id = " .$booking['id'];
    sql_command($sql);
  foreach ($compare  as $key => $value) {
    error_log("key: " . $key . ", places_id: " . $value);
    $sql = "INSERT INTO kth_entry_places(entry_id, places_id) 
          VALUES(" . $booking['id']. ", " . $value . ")";
    sql_command($sql);
  }
}
