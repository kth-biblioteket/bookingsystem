...
/* Ny tabell för att kunna välja tillgängliga platser för en tid */
...
/* Lägg till fält för defaultvärden i kth_places */
ALTER TABLE `kth_places` ADD `default_choice` INT NULL DEFAULT 0 AFTER `disabled`;
...
-- field_type: 0 = others/none, 1 = time, 2 = Input, 3 = text, 4 = Checkbox, 5 = Radio, 6 = Select
ALTER TABLE `kth_entry_custom_fields` ADD `field_type` int(11) NOT NULL AFTER `entry_field_name`;
ALTER TABLE `kth_entry_custom_fields` ADD `user_hidden` int(11) NOT NULL AFTER `enabled`;
...
(1, 'kth_campus', 6, 'custom', 422, 1, 1, 0, 1);
*/
