...
function create_field_entry_rooms($disabled=FALSE)
{
  ...
  //KTH visa inte (utom för admin, sätts i config)
  //191003 visa inte för någon
  //220531 visa för admin igen. ("batchbokningar för stängning")
...
function create_field_entry_confirmation_status($isbooking,$disabled=FALSE)
{
    ...
    //Visa tentativealternativet för admins.
    if ($is_admin) {
...
//191003
function create_field_entry_custom_field($field, $key, $is_admin, $user, $entry_id, $disabled=FALSE)
{
    ...
    {//TODO hårdkodning bort.
      ...
      case 'campus':
        ...
        */
        ...
        //Om det finns en rad i kth_entry_places för bokningen så ta inte hänsyn till vilka som är default_choice
          
        $entry_place_rows = FALSE;
        if ($entry_places_choice) {
          $sql_check_if_not_default = "SELECT kth_entry_places.entry_id
                                        FROM kth_entry_places 
                                        WHERE kth_entry_places.entry_id = $entry_id";
          
          $res_check_if_not_default = sql_query($sql_check_if_not_default);
          if (sql_count($res_check_if_not_default) > 0) {
            $entry_place_rows = TRUE;
          }
        }

        $sql = "SELECT kth_places.id, kth_places.name, kth_places.name_en, kth_entry_places.entry_id, kth_places.default_choice
          FROM kth_places
          LEFT JOIN kth_entry_places
          ON kth_places.id = kth_entry_places.places_id
          AND kth_entry_places.entry_id = $entry_id
          WHERE room_id = $room_id
          AND kth_places.disabled = 0
          ORDER BY kth_places.sort_key";
        ...
        for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
        {
          ...
          if ($row['name'] == '1none') {
            ...
            if($entry_place_rows) {
              if ($row['name'] != '1none' && (is_null($row['entry_id'])!=TRUE )) {
                if ($lang == "sv") {
                  $params['options'][$row['id']] = $row['name'];
                } else {
                  $params['options'][$row['id']] = $row['name_en'];
              ...
              if ($row['name'] != '1none' && (is_null($row['entry_id'])!=TRUE || $row['default_choice'] == TRUE )) {
                if ($lang == "sv") {
                  $params['options'][$row['id']] = $row['name'];
                } else {
                  $params['options'][$row['id']] = $row['name_en'];
        ...
        if ($is_admin && $entry_places_choice) {
          echo "<div style=\"border:1px solid;padding:5px;\" id=\"div_entry_places\">\n";
          echo "<label for=\"f_entry_places\">
            <div>ADMIN - Välj platser som ska visas för användaren för den här bokningen</div>
            <div>Väljer du bort alla så kommer de som är \"default\" att visas</div>
          </label>";

          $sql_entry_places = "SELECT kth_places.id, kth_places.name, kth_places.name_en, 
          kth_entry_places.entry_id, kth_entry_places.places_id, kth_places.default_choice
          FROM kth_places 
          LEFT JOIN kth_entry_places
          ON kth_places.id = kth_entry_places.places_id
          AND kth_entry_places.entry_id = $entry_id
          WHERE room_id = $room_id
          AND kth_places.disabled = 0
          AND  kth_places.name != '1none'
          ORDER BY kth_places.sort_key";
          
          $res_entry_places = sql_query($sql_entry_places);
          for ($i = 0; ($row_entry_places = sql_row_keyed($res_entry_places, $i)); $i++)
          {
            if($entry_place_rows) {
              if (is_null($row_entry_places['entry_id'])) {
                $entry_place_checked = "";
                $place_value = $row_entry_places['id'];
              } else {
                $place_value = $row_entry_places['id'];
                $entry_place_checked = "checked";
              }
            } else {
              if (is_null($row_entry_places['entry_id']) && $row_entry_places['default_choice'] != TRUE) {
                $entry_place_checked = "";
                $place_value = $row_entry_places['id'];
              } else {
                $place_value = $row_entry_places['id'];
            ...
            echo "<label class=\"secondary\">
            <input type=\"checkbox\" class=\"checkbox\" id=\"f_entry_places[]\" name=\"f_entry_places[]\" 
            value=\"" . $place_value ."\"" . $entry_place_checked . ">". $row_entry_places['name']. "</label>";
      ...
      default:
        echo "<div>\n";
        //Labelfält type = 7
        if ($custom_field_type["entry.$key"] == 7) { 
          echo "<label style=\"font-size:1.5rem\">" . get_vocab($key) . "</label>\n";
        } else {
          $fieldvalue = NULL;
          //191003
            ...
            (($field['nature'] == 'integer') && isset($field['length']) && ($field['length'] <= 2)) )
          {
            ...
            if ($custom_field_type["entry.$key"] == 5) {
              ...
              //Hämta från kth_radio_values
              //Sätt fieldvalue = default_value
              $options = get_custom_field_radio_values_array($key, $lang);
              $fieldvalue = get_custom_field_default_radio_choice($key);
...
$repeats_allowed = $is_admin && $default_bookings_open && $system_repeats_allowed;
...
foreach ($edit_entry_field_order as $key)
{
  ...
  case 'rooms':
    //220523 disabled = false för att aktivera val av (flera) rum
    create_field_entry_rooms(FALSE);
    break;
  ...
  if ($extended_booking_form==1) {
    //echo "<h4>Vänligen fyll i: </h4>";
	  ...
	  create_field_entry_custom_field($custom_fields_map[$key], $key, $is_admin, $user, $entry_id);
  }
  break;
  }
}
...
if (($edit_type == "series") && $repeats_allowed)
{
  ...
  echo "</fieldset>\n";
	...
	if ($id) {
		...
		if ((!empty($repeat_id) || $series) && $repeats_allowed)
		{
		  echo "<a class= \"btn btn-info deletebutton submit\" href=\"edit_entry.php?id=$id&amp;edit_type=series&amp;day=$day&amp;month=$month&amp;year=$year&amp;returl=$link_returl\">".get_vocab("editseries")."</a>";
		}
		echo "</div>\n";
