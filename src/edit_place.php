...
$place_default_choice = get_form_var('place_default_choice', 'string');
          ...
          if ($field['name'] != 'id')  // don't do anything with the id field
          {
            switch ($field['name'])
            {
              ...
              case 'disabled':
                ...
                break;
              case 'default_choice':
                $assign_array[] = "default_choice=$place_default_choice";
                break;
              case 'name':
...
if (isset($change_place) && !empty($place))
{
  ...
  <form class="form_general" id="edit_place" action="edit_place.php" method="post">
    ...
    <legend></legend>
      ...
      // Defaultchoice (yes or no)
      if ($is_admin)
      {
        echo "<div>\n";
        $options = array('0' => get_vocab("no"),
                         '1' => get_vocab("yes"));
        $params = array('label'         => get_vocab("Default choice") . ":",
                        'label_title'   => get_vocab("default_place_note"),
                        'name'          => 'place_default_choice',
                        'value'         => ($row['default_choice']) ? '1' : '0',
                        'options'       => $options,
                        'force_assoc'   => TRUE,
                        'disabled'      => $disabled,
                        'create_hidden' => FALSE);
